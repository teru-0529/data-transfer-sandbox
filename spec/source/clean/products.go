...
var ProductWhere = struct {
	...
	UpdatedBy:   whereHelpernull_String{field: "\"clean\".\"products\".\"updated_by\""},
}

// ProductRels is where relationship names are stored.
var ProductRels = struct {
	ProductNameOrderDetails string
}{
	ProductNameOrderDetails: "ProductNameOrderDetails",
}

// productR is where relationships are stored.
type productR struct {
	ProductNameOrderDetails OrderDetailSlice `boil:"ProductNameOrderDetails" json:"ProductNameOrderDetails" toml:"ProductNameOrderDetails" yaml:"ProductNameOrderDetails"`
}
...
func (r *productR) GetProductNameOrderDetails() OrderDetailSlice {
	if r == nil {
		return nil
	}
	return r.ProductNameOrderDetails
}
...
func (q productQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Product, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "clean: failed to execute a one query for products")
	}
...
func (q productQuery) All(ctx context.Context, exec boil.ContextExecutor) (ProductSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "clean: failed to assign all query results to Product slice")
	}
...
func (q productQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "clean: failed to count products rows")
	}
...
func (q productQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "clean: failed to check if products exists")
	}

	return count > 0, nil
}

// ProductNameOrderDetails retrieves all the order_detail's OrderDetails with an executor via product_name column.
func (o *Product) ProductNameOrderDetails(mods ...qm.QueryMod) orderDetailQuery {
...
// LoadProductNameOrderDetails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (productL) LoadProductNameOrderDetails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProduct interface{}, mods queries.Applicator) error {
	var slice []*Product
	var object *Product
	...
	if singular {
		object.R.ProductNameOrderDetails = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &orderDetailR{}
			}
			foreign.R.ProductNameProduct = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ProductName == foreign.ProductName {
				local.R.ProductNameOrderDetails = append(local.R.ProductNameOrderDetails, foreign)
				if foreign.R == nil {
					foreign.R = &orderDetailR{}
				}
				foreign.R.ProductNameProduct = local
				break
			}
		}
	}
...
// AddProductNameOrderDetails adds the given related objects to the existing relationships
// of the product, optionally inserting them as new records.
// Appends related to o.R.ProductNameOrderDetails.
// Sets related.R.ProductNameProduct appropriately.
func (o *Product) AddProductNameOrderDetails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OrderDetail) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ProductName = o.ProductName
			...
			values := []interface{}{o.ProductName, rel.OrderNo, rel.OrderDetailNo}
	...
	if o.R == nil {
		o.R = &productR{
			ProductNameOrderDetails: related,
		}
	} else {
		o.R.ProductNameOrderDetails = append(o.R.ProductNameOrderDetails, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &orderDetailR{
				ProductNameProduct: o,
			}
		} else {
			rel.R.ProductNameProduct = o
		}
	}
	return nil
}
...
func Products(mods ...qm.QueryMod) productQuery {
	mods = append(mods, qm.From("\"clean\".\"products\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"clean\".\"products\".*"})
	}
...
func FindProduct(ctx context.Context, exec boil.ContextExecutor, productName string, selectCols ...string) (*Product, error) {
	productObj := &Product{}
	...
	query := fmt.Sprintf(
		"select %s from \"clean\".\"products\" where \"product_name\"=$1", sel,
	)

	q := queries.Raw(query, productName)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "clean: unable to select from products")
	}
...
func (o *Product) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("clean: no products provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"clean\".\"products\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"clean\".\"products\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "clean: unable to insert into products")
	}
...
func (o *Product) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("clean: unable to update products, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"clean\".\"products\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to update products row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "clean: failed to get rows affected by update for products")
	}
...
func (q productQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to update all for products")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to retrieve rows affected for products")
	}
...
func (o ProductSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"clean\".\"products\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to update all in product slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to retrieve rows affected all in update all product")
	}
	return rowsAff, nil
}
...
func (o *Product) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
		return errors.New("clean: no products provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("clean: unable to upsert products, could not build update column list")
		}
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(productPrimaryKeyColumns) == 0 {
				return errors.New("clean: unable to upsert products, could not build conflict column list")
			}

			conflict = make([]string, len(productPrimaryKeyColumns))
			copy(conflict, productPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"clean\".\"products\"", updateOnConflict, ret, update, conflict, insert, opts...)
	...
	if err != nil {
		return errors.Wrap(err, "clean: unable to upsert products")
	}
...
func (o *Product) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("clean: no Product provided for delete")
	}
	...
	sql := "DELETE FROM \"clean\".\"products\" WHERE \"product_name\"=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to delete from products")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "clean: failed to get rows affected by delete for products")
	}
...
func (q productQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("clean: no productQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to delete all from products")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "clean: failed to get rows affected by deleteall for products")
	}
...
func (o ProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"clean\".\"products\" WHERE " +
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "clean: unable to delete all from product slice")
	}
...
func (o *Product) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindProduct(ctx, exec, o.ProductName)
	if err != nil {
		return err
	}
...
func (o *ProductSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"clean\".\"products\".* FROM \"clean\".\"products\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productPrimaryKeyColumns, len(*o))
	...
	if err != nil {
		return errors.Wrap(err, "clean: unable to reload all in ProductSlice")
	}
...
func ProductExists(ctx context.Context, exec boil.ContextExecutor, productName string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"clean\".\"products\" where \"product_name\"=$1 limit 1)"
	...
	row := exec.QueryRowContext(ctx, sql, productName)

	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "clean: unable to check if products exists")
	}
...
func (o *Product) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ProductExists(ctx, exec, o.ProductName)
}
